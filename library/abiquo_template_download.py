    ...
    if tpl is not None:
        if state == 'present' or state == 'download':
            ...
            try:
                ...
                module.fail_json(msg=ex.message)
            module.exit_json(
                msg='Template %s in datacenter %s deleted' %
                (template_name, dc_name), changed=True)
    else:
        if state == 'absent':
            module.exit_json(
                msg='Template %s in datacenter %s does not exist' %
                (template_name, dc_name), changed=False)
        elif state == 'download':
            try:
                task = template.download(
                    module, dc_name, remote_repository_url, template_name)
            except Exception as ex:
                module.fail_json(msg=ex.message)

            if module.params.get('wait_for_download'):
                try:
                    task = common.track_task(task, module.params.get(
                        'max_attempts'), module.params.get('retry_delay'))
                except Exception as ex:
                    module.fail_json(msg=ex.message)
                ...
                if task.state == 'FINISHED_SUCCESSFULLY':
                    ...
                    tpl_link = tpl._extract_link('edit')
                    module.exit_json(
                        msg='Template %s downloaded in datacenter %s' %
                        (template_name,
                         dc_name),
                        changed=True,
                        template=tpl.json,
                ...
                module.exit_json(
                    msg='Template %s downloading in datacenter %s' %
                    (template_name, datacenter), changed=True)
...
def main():
    ...
    try:
        ...
        module.fail_json(
            msg='Unanticipated error running abiquo_template_download: %s' %
